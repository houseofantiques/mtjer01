...
function isValidInternationalPhone(raw) {
  ...
  // يسمح + أرقام ومسافات ورموز بسيطة
...
function validateAndAlert() {
  ...
  if (nameParts.length < 3) {
    ...
    alert("الرجاء إدخال الاسم الكامل الثلاثي");
    els.orderName?.focus?.();
    return false;
  }
...
function bindLiveValidation() {
  ...
  requiredFields.forEach((el) => {
    ...
    el.addEventListener("blur", () => {
      ...
      if (el === els.orderPhone && !isValidInternationalPhone(c.phone)) {
        setInvalid(el);
      }

      if (el === els.orderGov && (!c.gov || c.gov.length < 2)) {
        setInvalid(el);
      }

      if (el === els.orderArea && (!c.area || c.area.length < 2)) {
        setInvalid(el);
      }

      if (el === els.orderEmail && !isValidEmail(c.email)) {
        setInvalid(el);
      }
    });
  });
 ...
 async function sendOrderEmail(p) {
  ...
  // 🔗 رابط القطعة (دومين GitHub Pages + hash)
  const productUrl = buildItemLink(p);

  // 🖼 صورة القطعة (HTTPS فقط)
  const productImage =
    (Array.isArray(p._images) && p._images.length > 0)
      ? p._images[0]
      : (p._image || "");

  const params = {
    /* ===== PRODUCT (مطابق للقالب) ===== */
    product_name: safeText(pickText(p, "name")),
    product_category: safeText(pickText(p, "category")),
    product_code: safeText(p._code),
    product_price: safeText(p.price) || formatIQD(p.priceNumber),
    product_url: safeText(productUrl),
    product_image_url: safeText(productImage),

    /* ===== CUSTOMER (مطابق للقالب) ===== */
