/* =========================================================
   ...
   - Validation + live field alerts
     Required: full name (3 parts), phone, email, governorate/city, area
     Optional: landmark, notes
   - Modal: Details/Order tabs + Sticky Top (shrinks on scroll)
   - Featured section + Auction toggle (persist)
   - Furniture category separated from Wood
   - Mobile back button closes modal (instead of exiting)
========================================================= */
...
(() => {
  ...
  // ✅ ضع رابط المتجر الحقيقي (GitHub Pages / Netlify) حتى الرابط يطلع صحيح داخل الإيميل
  ...
  function bindEls() {
    ...
    els.orderWhatsApp = $("orderWhatsApp"); // زر إرسال الطلب
  ...
  function buildItemLink(p) {
    // ✅ canonical link (no file:/// problems)
    try {
