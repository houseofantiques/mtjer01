...
const SHARE_WA_INTL = WHATSAPP_NUMBER_INTL;   // نفس الرقم للمشاركة
...
function getWhatsappUrl(numberIntl, text){
  return `https://wa.me/${numberIntl}?text=${encodeURIComponent(text)}`;
}
...
function sortProducts(list){
  ...
  if (s === "priceAsc"){
    copy.sort((a,b)=> (a.priceNumber ?? Number.POSITIVE_INFINITY) - (b.priceNumber ?? Number.POSITIVE_INFINITY));
    return copy;
  }

  if (s === "priceDesc"){
    copy.sort((a,b)=> (b.priceNumber ?? Number.NEGATIVE_INFINITY) - (a.priceNumber ?? Number.NEGATIVE_INFINITY));
    return copy;
  }
...
function applyStatusToModal(p){
  ...
  if (p.status === "sold"){
    ...
    els.orderSubmitBtn.style.opacity = "0.6";
    els.formStatus.textContent = "❌ هذه القطعة مباعة حالياً.";
  } else {
    els.orderSubmitBtn.disabled = false;
    els.orderSubmitBtn.style.opacity = "";
    if (p.status === "reserved"){
      els.formStatus.textContent = "⚠️ هذه القطعة محجوزة — تواصل للتأكد من التوفر.";
    } else {
      els.formStatus.textContent = "";
    }
  }
...
els.shareWAItemBtn.addEventListener("click", ()=>{
  ...
  if (!p) return;
  window.open(getWhatsappUrl(SHARE_WA_INTL, shareTextForProduct(p)), "_blank");
});
...
els.copyLinkBtn.addEventListener("click", async ()=>{
  ...
  try{
    ...
    els.formStatus.textContent = "تم نسخ رابط القطعة ✅";
  }catch{
    els.formStatus.textContent = "تعذر النسخ تلقائياً. انسخي الرابط يدوياً.";
  }
});
  ...
  if (navigator.share){
    try{
      await navigator.share({
        title: p.name,
        text: shareTextForProduct(p),
        url
      });
    }catch{}
  } else {
    // إذا الجهاز ما يدعم share
    try{
      await navigator.clipboard.writeText(url);
      els.formStatus.textContent = "جهازك لا يدعم زر المشاركة — تم نسخ الرابط ✅";
    }catch{
      els.formStatus.textContent = "جهازك لا يدعم زر المشاركة.";
    }
  }
});
...
els.form.addEventListener("submit", (e)=>{
  ...
  if (p.status === "sold"){
    els.formStatus.textContent = "❌ هذه القطعة مباعة ولا يمكن طلبها.";
    return;
  }
  ...
  if (!name || !phone || !gov || !area){
    els.formStatus.textContent = "رجاءً اكملي الاسم، الهاتف، المحافظة، والمنطقة.";
    return;
  }
  ...
  const orderText = buildOrderText({
    ...
    pageUrl: getProductUrl(p.id)
  });

  window.open(getWhatsappUrl(WHATSAPP_NUMBER_INTL, orderText), "_blank");
...
function openQuickWhatsApp(){
  ...
  window.open(getWhatsappUrl(WHATSAPP_NUMBER_INTL, msg), "_blank");
}
...
/* =========================
   فتح قطعة من الرابط (Hash)
   مثال: #item=HOA-WOOD-004
========================= */
